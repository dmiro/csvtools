...
class QCsv(QTableView):
    ...
    def modifiedValue(self):
        modifiedDateime = QDateTime(datetime.fromtimestamp(os.path.getmtime(self.filename)))
        strDateTime = modifiedDateime.toString(Qt.SystemLocaleShortDate)
        return unicode(strDateTime)
        #return datetime.fromtimestamp(os.path.getmtime(self.filename)).strftime('%d/%m/%y %H:%M:%S')
    ...
    def search(self, text, matchMode, matchCaseOption):
        ...
        # search    
        for numRow, dataRow in enumerate(self.data):
    ...
    def __init__(self, filename, data, *args):
